...
import com.efacility.ticketing.service.TiketPristup;
...
public class TiketServiceImpl implements TiketService {
    ...
    public TiketServiceImpl(TiketRepository ticketRepository,
    ...
    public TiketDTO createTiket(CreateTiketRequest request, Korisnik currentKorisnik) {
        Stan apartment = apartmentRepository.findByIdAndTenantId(request.getApartmentId(), currentKorisnik.getId())
                .orElseThrow(() -> new TiketAccessDeniedException(
                        "Možete prijaviti kvar samo za stan koji vam je dodeljen."));
