...
interface Props {
  ticket: TicketDTO;
  onClose: () => void;
  onAssigned: (ticket: TicketDTO) => void;
}

export default function AssignTechnicianModal({ ticket, onClose, onAssigned }: Props) {
  ...
  useEffect(() => {
    userService.getTechnicians().then(setTechnicians).catch((err) => setError(getErrorMessage(err, 'Failed to load technicians.')));
  }, []);
  ...
  async function handleAssign() {
    ...
    setError('');
    try {
      const updated = await ticketService.assignTechnician({ ticketId: ticket.id, technicianId: selectedId });
      onAssigned(updated);
    } catch (err) {
